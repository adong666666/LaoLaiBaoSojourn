...
class histroicalViewController: UIViewController {
    ...
    var btn: PowerButton = {() in
        ...
        bt1.addTarget(self, action: #selector(histroicalViewController.buttonTap(_:)), for: UIControl.Event.touchUpInside)
        return bt1
    }()
...
extension histroicalViewController{
    func allthings(){
        super.viewDidLoad()
        addSubview()
        delegate()
...
extension histroicalViewController: UITableViewDelegate, UITableViewDataSource{
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return 5
    }
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: CellID) as! histroicalorderTableViewCell
        cell.awakeFromNib()

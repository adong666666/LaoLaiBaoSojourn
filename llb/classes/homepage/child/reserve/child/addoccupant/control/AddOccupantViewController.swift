...
class addoccupantViewController: UIViewController {
    ...
    var btn2: PowerButton = {
        ...
        bt1.addTarget(self, action: #selector(reserveViewController.buttonTap2(_:)), for: UIControl.Event.touchUpInside)
        return bt1
    }()
...
extension addoccupantViewController{
    func allthings(){
        super.viewDidLoad()
        addSubview()
    }
...
extension addoccupantViewController{
    ...
    @objc func buttonTap3(_ button: UIButton){
        self.navigationController?.pushViewController(occupantinfoViewController(), animated: true)
    }
}
extension addoccupantViewController{
